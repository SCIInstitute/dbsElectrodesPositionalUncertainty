% Find most probable electrode trajectory by fine sampling of octant around
...
function sortedTraj = fineRefineTrajectoryMC(estimatedDirSVD,imElectrodeStretch, dwnSmpRate)

[estPhi, estTheta] = getSphericalCoord(estimatedDirSVD);

% Monte carlo samples for octant
var = 10;
phiArr = max(0,estPhi-var):0.05:min(estPhi+var,50);
if(estimatedDirSVD(1) > 0 && estimatedDirSVD(2) > 0)
    thetaArr = max(estTheta-var,1):0.05:min(estTheta+var,89);
elseif(estimatedDirSVD(1) < 0 && estimatedDirSVD(2) > 0)
    thetaArr = max(estTheta-var,91):0.05:min(estTheta+var,179);
elseif(estimatedDirSVD(1) < 0 && estimatedDirSVD(2) < 0)
    thetaArr = max(estTheta-var,181):0.05:min(estTheta+var,269);
elseif(estimatedDirSVD(1) > 0 && estimatedDirSVD(2) < 0)
    thetaArr = max(estTheta-var,271):0.05:min(estTheta+var,359);
end
